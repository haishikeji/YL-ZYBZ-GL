        ...
        <el-col :span="12">
            <el-form-item label="上级目录" prop="parent.id"
                ...
                <SelectTree
                      ...
                      v-if="visible"
                      :url="`/zy/medicineclass/medicineClass/treeData?extId=${inputForm.id}`"
...
<script>
  ...
  import MedicineClassService from '@/api/zy/medicineclass/MedicineClassService'
  export default {
    data () {
      return {
        title: '',
    ...
    medicineClassService: null,
    created () {
      this.medicineClassService = new MedicineClassService()
    },
    methods: {
      init (method, obj) {
        this.method = method
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.medicineClassService.queryById(this.inputForm.id).then(({data}) => {
      ...
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            this.medicineClassService.save(this.inputForm).then(({data}) => {
              this.visible = false
